...
class TestCLIInvocationWithProfilesDir(ModelCopyingIntegrationTest):
    ...
    def setUp(self):
        ...
        self.oracle_safe_drop_user(self.custom_schema)
        self.run_sql(f'CREATE USER {self.custom_schema} IDENTIFIED BY {self.custom_schema}')
        self.run_sql(f'GRANT ALL PRIVILEGES TO {self.custom_schema}')
    ...
    def custom_schema(self):
        return "{}_custom".format(self.unique_schema())[:30]
