...
class TestPrePostRunHooks(DBTIntegrationTest):
    ...
    def project_config(self):
        return {
            ...
            "on-run-start": [
                ...
                "create table {{ target.schema }}.start_hook_order_test ( id number )",
                "drop table {{ target.schema }}.start_hook_order_test",
            ],
            "on-run-end": [
                "{{ custom_run_hook('end', target, run_started_at, invocation_id) }}",
                "create table {{ target.schema }}.end_hook_order_test ( id number )",
                "drop table {{ target.schema }}.end_hook_order_test",
                "create table {{ target.schema }}.schemas ( schema varchar2(4000 char) )",
                "insert into {{ target.schema }}.schemas (schema) values {% for schema in schemas %}( '{{ schema }}' ){% if not loop.last %},{% endif %}{% endfor %}",
                "create table {{ target.schema }}.db_schemas ( db varchar2(4000 char), schema varchar2(4000 char) )",
    ...
    def check_hooks(self, state):
        ...
        self.assertEqual(ctx['target.dbname'], 'XEPDB1')
        self.assertEqual(ctx['target.host'], 'localhost')
        ...
        self.assertEqual(ctx['target.user'], 'SYS')
        self.assertEqual(ctx['target.pass'], None)
