...
class TestSimpleDependencyHooks(DBTIntegrationTest):
    ...
    def project_config(self):
        ...
        return {
            'config-version': 2,
            'on-run-start': [
                oracle_drop_statement.format("{{ var('test_create_table') }}"),
                oracle_drop_statement.format("{{ var('test_create_second_table') }}")
            ]
        }
