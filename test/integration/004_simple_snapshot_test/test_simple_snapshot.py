...
class BaseSimpleSnapshotTest(DBTIntegrationTest):
    ...
    def dbt_run_seed_snapshot(self):
        # ORACLE FIX
        if self.adapter_type in ('postgres', 'oracle'):
...
class TestSimpleColumnSnapshotFiles(DBTIntegrationTest):
    ...
    def _run_snapshot_test(self):
        ...
        results = self.run_sql(
            'select * from {}.my_snapshot'.format(self.unique_schema()),
            fetch='all'
        )
        ...
        results = self.run_sql(
            'select * from {}.my_snapshot where last_name is not NULL'.format(self.unique_schema()),
            fetch='all'
        )
        self.assertEqual(len(results), 3)
        ...
        results = self.run_sql(
            'select * from {}.my_snapshot where last_name is NULL'.format(self.unique_schema()),
            fetch='all'
        )
        self.assertEqual(len(results), 3)
        for result in results:
...
class TestSimpleSnapshotFileSelects(DBTIntegrationTest):
    ...
    @use_profile('postgres')
    def test__postgres__snapshots(self):
        self.run_sql_file('seed_pg.sql')
...
class TestCrossSchemaSnapshotFiles(DBTIntegrationTest):
    ...
    def target_schema(self):
        ...
        return "{}_snapshotted".format(self.unique_schema()[:18])
    ...
    def test__postgres__long_text(self):
        ...
        with self.adapter.connection_named('test'):
            status, results = self.adapter.execute(
                'select * from {}.snapshot_actual'.format(self.unique_schema()),
                fetch=True
            )
...
class TestSlowQuery(DBTIntegrationTest):
    ...
    def test__postgres__slow(self):
        ...
        self.run_dbt(['run'])  # need to materialize gen.sql as table
...
class TestSnapshotHardDelete(DBTIntegrationTest):
    ...
    def _delete_records(self):
        ...
        self.run_sql(
            'delete from {}.seed where id >= 10;'.format(self.unique_schema())
        )
    ...
    def _revive_records(self):
        ...
        self.run_sql(
            '''
            insert into {0}.seed (id, first_name, last_name, email, gender, ip_address, updated_at) values (10, 'Rachel', 'Lopez', 'rlopez9@themeforest.net', 'Female', '237.165.82.71', '{1}');
            insert into {0}.seed (id, first_name, last_name, email, gender, ip_address, updated_at) values (11, 'Donna', 'Welch', 'dwelcha@shutterfly.com', 'Female', '103.33.110.138', '{1}');
            '''.format(self.unique_schema(), revival_timestamp)
        )
