...
class TestExternalReference(DBTIntegrationTest):
    ...
    def setUp(self):
        ...
        self.run_sql('create user "{0}" IDENTIFIED BY {0}'.format(self.external_schema))
        self.run_sql('grant all privileges to "{}"'.format(self.external_schema))
        self.run_sql(
            'create table "{}"."external" (id integer)'
                .format(self.external_schema)
        )
        self.run_sql(
            'insert into "{}"."external" values (1)'.format(self.external_schema)
        )
        self.run_sql(
            'insert into "{}"."external" values (2)'.format(self.external_schema)
        )

    def tearDown(self):
        self.run_sql('DROP USER "{}" CASCADE'.format(self.unique_schema() + 'z'))
...
class TestExternalDependency(DBTIntegrationTest):
    ...
    def tearDown(self):
        self.run_sql('DROP USER "{}" CASCADE'.format(self.unique_schema() + 'zz'))
        # This has to happen before we drop the external schema, because
        # otherwise postgres hangs forever.
        self._drop_schemas()
        with self.get_connection():
